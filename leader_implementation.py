...
class LeaderAlgorithms:
    ...
    def __init__(self, data, varinfo, weights=None, similarity_func = 'gower', seed=None, verbose = 1):
        ...
        ----------
        data : pd.DataFrame
            The input dataset. Each row represents an observation, and each column represents a feature.
        varinfo : pd.DataFrame
            DataFrame containing metadata about the variables in the dataset. Required columns: ['name', 'type'] 
            where 'type' can be ['Binary', 'Categorical', 'Integer', 'Continuous']. If a column 'role' is present, 
            only rows with 'role' == 'Feature' will be considered.
        weights : list or np.ndarray, optional
            Weights for each feature in the dataset. If None, uniform weight will be applied. Default is None.
        similarity_func : str, optional
            The similarity function to use, either 'gower' or 'euclidean'. Default is 'gower'.
        seed : int, optional
            Random seed for reproducibility. Default is None.
        verbose : int, optional
            Verbosity level. If 0, no output will be shown. Default is 1.
        """
        if seed != None:
            np.random.seed(seed)
    ...
    def _Leader2(data, s_min, types, ranges, weights, pairwise_function):
        """
        Implement Hernández's modification of Hartigan's Leader algorithm to cluster data points.
    ...
    def Leader2(self, s_min, data = None, verbose=1):
        """
        Applies Hernández's modification of Hartigan's Leader algorithm to the dataset to form clusters based on a similarity threshold.
    ...
    def Leader_Medoid(self, s_min, data = None, parallel=False, verbose=1):
        """
        Executes the Leader Medoid algorithm to form clusters based on a similarity threshold.
    ...
    def _Leader2_Medoid(data, s_min, types, ranges, weights, pairwise_function, parallel=False, merge=False):
        """
        Implement the Hernández's Leader algorithm with medoid adjustment to cluster data points.
    ...
    def Leader2_Medoid(self, s_min, data=None, parallel=False, merge=False, verbose=1):
        """
        Executes the Leader2 Medoid algorithm to form clusters based on a similarity threshold.
        ...
        parallel : bool, optional
            Whether to use parallel computation (default is False).
        merge : bool, optional
            Whether to merge clusters with similarity above s_min (default is False).
        verbose : int, optional
            Verbosity level. If 0, no output will be shown. Default is 1.
        ...
        elapsed_time = time.time() - start_time
        if verbose != 0:
            self.summary_of_statistics("Leader2 Medoid", elapsed_time)  # Print summary statistics
    ...
    def Leader3_Medoid(self, s_min, data=None, parallel=False, merge=False, second_pass=False, verbose=1):
        """
        Executes the Leader3 Medoid algorithm to form clusters based on a similarity threshold.
        ...
        second_pass : bool, optional
            Whether to perform a second pass clustering (default is False).
        verbose : int, optional
            Verbosity level. If 0, no output will be shown. Default is 1.
        ...
        elapsed_time = time.time() - start_time
        if verbose != 0:
            self.summary_of_statistics("Leader3 Medoid", elapsed_time)  # Print summary statistics
    ...
    def plot_graph(self, figsize=(16, 8), edges_strat='top_n', n=-1):
        """
        Visualizes the clusters formed by the Leader Medoid algorithm using a graph representation.

        Parameters:
        ----------
        figsize : tuple, optional
            Figure size (width, height) in inches (default is (16, 8)).
        edges_strat : {'top_n', 'all'}, optional
            Strategy for selecting edges in the graph:
            - 'top_n': Selects top n edges based on similarity (default).
            - 'all': Uses all possible edges.
        n : int, optional
            Number of top edges to select when edges_strat is 'top_n' (default is -1, which selects all leaders).
    ...
    def plot_clustering_summary(self):
        """
        Visualizes a summary of the clustering results including cluster sizes, average leader similarity,
        and similarity heatmap between cluster leaders.

        Returns:
        -------
        None
            Displays the clustering summary visualization.
        """
        data = self.data
        cluster_sizes = self.cluster_sizes
        avg_sim = self.avg_leader_sim
    ...
    def summary_of_statistics(self, algorithm, elapsed_time):
        """
        Prints a summary of statistics including the number of clusters found, each cluster's leader,
        size, and average similarity.
        ...
        elapsed_time : float
            The elapsed time taken to execute the algorithm.

        Returns:
        -------
        None
            Prints the summary of statistics to the console.
        """
    ...
    def cluster_boxplots(self):
        """
        Generates boxplots of similarity scores between each data point and its respective cluster leader.

        Returns:
        -------
        None
            Displays the boxplots of similarity scores.
        """
        data = self.data
    ...
    def simplified_silhouette(self):
        """
        Computes simplified silhouette scores for each data point based on its cluster assignment and similarity to
        cluster leaders.

        Returns:
        -------
        np.ndarray
            Array of simplified silhouette scores for each data point.
        """
        leaders = self.leaders
        data = self.data
        cluster = self.cluster
    ...
    def plot_silhouette(self):
        """
        Plots the silhouette plot for clusters based on simplified silhouette coefficients.

        Returns:
        -------
        None
            Displays the silhouette plot and prints average silhouette widths for each cluster and the entire dataset.
        """
...
def _find_most_similar_leader(item, leaders, data, types, ranges, weights, pairwise_function):
    """
    Find the most similar leader for a given data point.
    ...
    leaders : list
        List of indices representing current leaders for each cluster.
    data : np.ndarray
        Array containing the dataset.
    types : list
        List of data types for each feature in the dataset.
    ranges : list
        List of ranges (min, max) for each feature in the dataset.
    weights : list
        List of weights for each feature in the dataset.
    pairwise_function : callable
        Function to compute pairwise similarity between two data points.

    Returns:
    --------
    float
        Maximum similarity found for the current data point.
    int
        Index of the leader with the maximum similarity.
    int
        Cluster index of the leader with the maximum similarity.
    """
...
def _find_medoid_parallel(item, leader_idx, sim_item_leader, leaders, cluster, avg_sim, avg_leader_sim, cluster_sizes, data, types, ranges, weights, pairwise_function):
    """
    Update leaders and average similarity metrics in parallel for adding a new data point.

    Parameters:
    -----------
    item : int
        Index of the new data point to be added.
    leader_idx : int
        Index of the current leader (medoid) of the cluster to which the new data point belongs.
    sim_item_leader : float
        Similarity score between the new data point and the current leader.
    leaders : list
        List of indices representing current leaders for each cluster.
    cluster : np.ndarray
        Array containing cluster assignments for each data point.
    avg_sim : np.ndarray
        Array storing average similarity metrics for each data point.
    avg_leader_sim : np.ndarray
        Array storing average similarity metrics for each cluster leader.
    ...
    --------
    tuple
        Updated leaders, avg_sim, avg_leader_sim, and cluster_sizes after adding the new data point.
    """
...
def _remove_find_medoid_parallel(item, leader_idx, sim_item_leader, leaders, cluster, avg_sim, avg_leader_sim, cluster_sizes, data, types, ranges, weights, pairwise_function):
    """
    Update leaders and average similarity metrics in parallel for removing a data point.

    Parameters:
    -----------
    item : int
        Index of the data point to be removed.
    leader_idx : int
        Index of the current leader (medoid) of the cluster from which the data point is removed.
    sim_item_leader : float
        Similarity score between the data point to be removed and the current leader.
    leaders : list
        List of indices representing current leaders for each cluster.
    cluster : np.ndarray
        Array containing cluster assignments for each data point.
    ...
    --------
    tuple
        Updated leaders, avg_sim, avg_leader_sim, and cluster_sizes after removing the data point.
    """
    k = cluster[leader_idx]
    # Update the average similarity for the cluster leader
    ...
    # Decrement the size of the cluster as a data point has been removed
    cluster_sizes[k] -= 1 
...
def _find_medoid_sequential(item, leader_idx, sim_item_leader, leaders, cluster, avg_sim, avg_leader_sim, cluster_sizes, data, types, ranges, weights, pairwise_function):
    """
    Update leaders and average similarity metrics sequentially for adding a new data point.
...
def _remove_find_medoid_sequential(item, leader_idx, sim_item_leader, leaders, cluster, avg_sim, avg_leader_sim, cluster_sizes, data, types, ranges, weights, pairwise_function):
    """
    Update leaders and average similarity metrics sequentially for removing a data point.
...
def _merge_medoid(N, i, j, leaders, cluster, cluster_sizes, data, types, ranges, weights, pairwise_function):
    """
    Merge two clusters by finding a new leader and updating cluster assignments.
    ...
    --------
    int
        Index of the new leader after merging.
    int
        Index of the cluster (k) to which the new leader belongs.
    """
    max_sim = -1
    new_leader = -1
    ...
    # Iterate over all data points to find a new leader with maximum average similarity
    for l in range(N):
...
def _Reassigner_Leader(leaders, cluster, avg_leader_sim, cluster_sizes, data, types, ranges, weights, pairwise_function):
    """
    Reassign cluster leaders and update average similarity metrics after initialization.
    ...
    --------
    tuple
        Updated leaders, cluster assignments, avg_leader_sim, and cluster_sizes after reassignment.
    """
    # Number of data points
    N = len(data)

    # Reinitialize cluster assignments, leaders, and related statistics
